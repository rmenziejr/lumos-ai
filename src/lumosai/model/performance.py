...
def performance_report(
    ...
    When omitted, ``include_plots=True`` preserves the legacy behavior of rendering
    all plots applicable to the resolved task. An explicit ``plots`` list takes
    precedence over ``include_plots``.
    """
    ...
    selected_categorical_columns = validate_categorical_columns(
        current_pd, categorical_columns=categorical_columns, analysis_columns=feature_columns
    )
    resolved_task = task_type or detect_task_type(current_pd[target], current_pd[prediction])
    selected_plots = _resolve_plots(
        plots=plots, include_plots=include_plots, task_type=resolved_task
    )
    scores = (
    ...
    raw_metrics = get_metrics(
        ...
        task_type=resolved_task, custom_metrics=custom_metrics, positive_label=positive_label,
    )
    summary: dict[str, Any] = {"rows": len(current_pd), "metrics": raw_metrics}
    lift_summary: dict[str, Any] | None = None
    needs_lift_data = bool({"lift", "capture"} & selected_plots)
    if include_lift:
    ...
    elif resolved_task == "classification" and scores is not None and needs_lift_data:
    ...
    if train is not None:
        ...
        validate_prediction_frame(
            train_pd, target=target, prediction=prediction,
            prediction_score=prediction_score if isinstance(prediction_score, str) else None,
        )
        if feature_columns is not None:
            require_columns(train_pd, feature_columns)
        validate_categorical_columns(
            train_pd, categorical_columns=categorical_columns, analysis_columns=feature_columns
        )
        train_scores = (
            normalize_classification_scores(
                train_pd, target=target, prediction=prediction,
                prediction_score=prediction_score, score_labels=score_labels,
            )
            if resolved_task == "classification" and prediction_score is not None else None
        )
        _set_binary_positive_label(train_scores, positive_label)
        train_raw_metrics = get_metrics(
            train_pd[target], train_pd[prediction],
    ...
    metrics = (
        _comparative_performance_metrics(train_metrics=train_raw_metrics, holdout_metrics=raw_metrics)
        if train_raw_metrics is not None
        else {f"performance/{name}": value for name, value in raw_metrics.items()}
    )
    metadata: dict[str, Any] = {
        "report_type": "performance", "task_type": resolved_task,
    ...
    if train_raw_metrics is not None:
        ...
        summary["comparison"] = _comparative_performance_summary(
            train_metrics=train_raw_metrics, holdout_metrics=raw_metrics
        )
    ...
    if selected_plots:
        ...
        with artifact_workspace(keep_local=keep_local) as workspace:
            html_path = local_html_artifact_path(workspace, "performance_report.html", report_name=report_name)
            html_path.write_text(
                performance_html(
                    title=title, frame=current_pd, target=target, prediction=prediction,
                    task_type=resolved_task, metrics=metrics, scores=scores,
                    lift_summary=lift_summary, plots=selected_plots,
                ), encoding="utf-8",
            )
            artifacts, _ = html_artifact_metadata(
                html_path, artifact_path="performance", experiment_name=experiment_name
            )
            ...
            return log_result_with_html_artifact(
                result, html_path=html_path, artifact_path="performance", experiment_name=experiment_name
            )
    result = LumosResult(metrics=metrics, summary=summary, artifacts=artifacts, metadata=metadata)
...
def _comparative_performance_summary(*, train_metrics: dict[str, float], holdout_metrics: dict[str, float]) -> dict[str, dict[str, float]]:
    ...
    for name in sorted(set(train_metrics).intersection(holdout_metrics)):
        comparison[name] = {
            "train": train_metrics[name], "holdout": holdout_metrics[name],
            "gap": _metric_gap(metric=name, train_value=train_metrics[name], holdout_value=holdout_metrics[name]),
            "ratio": _metric_ratio(train_value=train_metrics[name], holdout_value=holdout_metrics[name]),
        }
    return comparison
